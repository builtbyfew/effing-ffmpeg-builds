...
All builds are configured with `--enable-gpl --enable-libx264 --enable-mbedtls` (mbedTLS provides HTTPS support). The Linux binaries are fully static (musl-linked) with no external libc dependency, so they run on any Linux distro. The Windows binary is fully static (no DLL deps) as well.
